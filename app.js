/* Kitobchi — mobil UI, ko‘rinadigan AI ish jarayoni va tiklanadigan topshiriqlar */
(() => {
  ...
  const getJob = () => KitobchiStore.jobs.get();
  const setJob = patch => { const job = {...(getJob() || {}), ...patch, updatedAt: Date.now()}; KitobchiStore.jobs.set(job); currentJob = job; render(); };
  const clearJob = () => { KitobchiStore.jobs.clear(); currentJob = null; };

  function render() {
    back.style.visibility = route === 'home' ? 'hidden' : 'visible';
    settingsBtn.style.visibility = route === 'settings' ? 'hidden' : 'visible';
    currentJob = getJob();
    if (route === 'home') home(); else if (route === 'create') create(); else if (route === 'workspace') workspace(); else settings();
  }
  function nav(activeName = 'home') { return `<nav class="bottom-nav"><button class="${activeName === 'home' ? 'active' : ''}" id="navHome">⌂<small>Bosh sahifa</small></button><button class="${activeName === 'settings' ? 'active' : ''}" id="navSettings">⚙<small>Sozlamalar</small></button></nav>`; }
  function bindNav() { document.querySelector('#navHome')?.addEventListener('click', () => { route = 'home'; render(); }); document.querySelector('#navSettings')?.addEventListener('click', () => { route = 'settings'; render(); }); }
  function home() {
    const ps = KitobchiStore.projects.list();
    const job = currentJob && currentJob.status === 'running' ? `<div class="job-mini"><span class="spinner"></span><div><b>AI ishlayapti</b><small>${esc(currentJob.stage || 'Tayyorlanmoqda')} · ${currentJob.progress || 0}%</small></div><button id="resumeJob">Ko‘rish</button></div>` : '';
    screen.innerHTML = `<section class="hero"><p class="eyebrow">IJOD UCHUN MAYDON</p><h1>G‘oyangizni<br><em>kitobga aylantiring.</em></h1><p class="muted">AI bilan yozing, bezang va birgina bosishda ulashing.</p><button class="primary wide" id="new">＋ Yangi loyiha</button></section>${job}<div class="section-head"><h2>Loyihalar</h2><span>${ps.length}</span></div><div class="cards">${ps.length ? ps.map(p => `<button class="project-card" data-id="${p.id}"><span class="project-icon">${p.type === 'book' ? '▤' : p.type === 'slides' ? '▥' : '▧'}</span><span><b>${esc(p.title)}</b><small>${esc(p.typeLabel || 'Kitob')} · ${new Date(p.updatedAt || Date.now()).toLocaleDateString('uz-UZ')}</small></span><i>›</i></button>`).join('') : `<div class="empty"><div>✦</div><p>Hali loyiha yo‘q</p><small>Birinchi g‘oyangizni hoziroq boshlang.</small></div>`}</div><div class="quick-features"><div><span>✦</span><b>AI suhbat</b><small>G‘oyani aniqlaydi</small></div><div><span>▥</span><b>Slaydlar</b><small>PPT tayyorlaydi</small></div><div><span>▣</span><b>Eksport</b><small>PDF va HTML</small></div></div>${nav()}`;
    document.querySelector('#new').onclick = () => { route = 'create'; render(); }; document.querySelector('#resumeJob')?.addEventListener('click', () => { active = KitobchiStore.projects.get(currentJob.projectId); route = 'workspace'; render(); });
    document.querySelectorAll('[data-id]').forEach(x => x.onclick = () => { active = KitobchiStore.projects.get(x.dataset.id); route = 'workspace'; render(); }); bindNav();
  }
  function create() {
    screen.innerHTML = `<section class="page"><p class="eyebrow">YANGI LOYIHA</p><h1>Nima yaratamiz?</h1><p class="muted">G‘oyangizni yozing. Kitobchi uni savollar bilan aniq loyihaga aylantiradi.</p><div class="type-grid"><button class="type selected" data-type="book">📖<b>Kitob</b><small>Hikoya, roman, qo‘llanma</small></button><button class="type" data-type="slides">▥<b>Slaydlar</b><small>Ta’lim va taqdimot</small></button><button class="type" data-type="doc">▧<b>Hujjat</b><small>Universitet va biznes</small></button></div><label class="label">G‘oyangiz yoki buyruqingiz<textarea id="prompt" placeholder="Masalan: Bolalar uchun kosmos haqida rangli ertak..."></textarea></label><button class="primary wide" id="start">AI bilan boshlash →</button></section>`;
    let type = 'book'; document.querySelectorAll('.type').forEach(b => b.onclick = () => { document.querySelectorAll('.type').forEach(x => x.classList.remove('selected')); b.classList.add('selected'); type = b.dataset.type; });
    document.querySelector('#start').onclick = () => { const p = document.querySelector('#prompt').value.trim(); if (!p) return toast('Avval g‘oyangizni yozing.'); active = {id:id(), title:p.slice(0,36), type, typeLabel:type === 'book' ? 'Kitob' : type === 'slides' ? 'Slaydlar' : 'Hujjat', prompt:p, updatedAt:Date.now(), messages:[], pages:[]}; KitobchiStore.projects.create(active); route = 'workspace'; render(); };
  }
  ...
  async function send() {
    const input = document.querySelector('#msg'), text = input.value.trim(); if (!text || (currentJob && currentJob.status === 'running')) return;
    active.messages = active.messages || []; active.messages.push({role:'user', text}); saveActive();
    setJob({id:id(), projectId:active.id, kind:'chat', status:'running', stage:'Buyruqni tahlil qilish', progress:8, step:0, prompt:text, baseMessages:active.messages.slice(0,-1)});
  ...
  back.onclick=()=>{ if(route!=='home'){route='home';render();} }; settingsBtn.onclick=()=>{route='settings';render();}; window.addEventListener('online',resumeJob); document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible') resumeJob();}); render(); setTimeout(resumeJob,250);
})();